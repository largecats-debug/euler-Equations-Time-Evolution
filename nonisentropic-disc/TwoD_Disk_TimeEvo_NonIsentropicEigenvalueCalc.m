...
function [lambda,gamma,beta,fLoopReturn,gammaFunctions] = TwoD_Disk_TimeEvo_NonIsentropicEigenvalueCalc(rPoints,rGrid, ...
    radialEntropyLevels,entropyLevelWidths,entropyLevelStartingIndex,sigmaBarProfile,thetaBasisSize, ...
    rBasisSize,besselZeroDataTable,besselDerivativeZeroDataTable)
...
gammaFunctions = cell(thetaBasisSize,radialEntropyLevels);
...
for n1 = 1:thetaBasisSize
    ...
    for m1 = 1:rBasisSize
        ...
        for i1 = 2:radialEntropyLevels
            ...
            fLoopIndex{i1-1} = @(x) intervalOfAngle(fLoop{i1-1}(x),n1,besselZeroDataTable);
            
            fLoopInverse{i1-1} = @(x) besselLogDerivativeInverse(n1,(sigmaBarProfile(i1-1)/sigmaBarProfile(i1))*fLoop{i1-1}(x),fLoopIndex{i1-1}(x),besselLogDerivativeFnc,besselZeroDataTable);

            gammaFunctions{n1,i1} = @(x) rGrid(entropyLevelStartingIndex(i1)) - (1/(x*sigmaBarProfile(i1))) * fLoopInverse{i1-1}(x);

            if i1 < radialEntropyLevels
                fLoop{i1} = @(x) besselLogDerivativeFnc(x*sigmaBarProfile(i1)*(rGrid(entropyLevelStartingIndex(i1+1))-gammaFunctions{n1,i1}));
            end
