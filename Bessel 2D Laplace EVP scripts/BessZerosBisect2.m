function [jprimemk,JofJ]=BessZerosBisect2(MM,KK,tol)
    ...
    for k1=1:len_k
        ...
        if((m+1 <= row_bdz) && (k <= col_bdz))
            ...
            if k == 1
                ...
                betapr=(m/2+k-3/4)*pi;
                %betapr=(m+k/2-3/4)*pi; %changed according to DLMF resource?
                mu=4*m^2;
                asymptroot=betapr-(mu+3)/(8*betapr);
                asymptroot=asymptroot-4*(7*mu^2+82*mu-9)/(3*(8*betapr)^3);
                asymptroot = asymptroot - 32*(83*mu^3+2075*mu^2-3039*mu+3537)/(15*(8*betapr)^5);
            end
